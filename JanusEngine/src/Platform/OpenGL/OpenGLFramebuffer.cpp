...
namespace Janus {
	...
	OpenGLFramebuffer::OpenGLFramebuffer(const FramebufferSpecification& spec) : m_Specifcation(spec) {
		for (auto spec : m_Specifcation.Attachments.Attachments) {
	...
	void OpenGLFramebuffer::Invalidate() {
		...
		bool multisample = m_Specifcation.Samples > 1;
		...
		if (m_DepthAttachmentSpec.TextureFormat != FramebufferTextureFormat::None) {
			...
			case FramebufferTextureFormat::DEPTH24STENCIL8:
				Utils::AttachDepthTexture(m_DepthAttachment, m_Specifcation.Samples, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT, m_Specifcation.Width, m_Specifcation.Height);
				break;
			}
		}

		if (m_ColorAttachments.size() > 1) {
			JN_CORE_ASSERT(m_ColorAttachments.size() <= 4);
